...
export function RegioesEntregaModal({ isOpen, onClose }: Props) {
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault()
    if (!nome) return
    addRegiaoEntrega({ nome, taxa: parseFloat(taxa) || 0, prazoMins: parseInt(prazo) || 0, status: 'Ativo' })
    setNome("")
    setTaxa("")
    setPrazo("")
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[600px] rounded-[1.5rem] p-6">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Regiões de Entrega</DialogTitle>
        </DialogHeader>
        
        <div className="mt-4 flex flex-col gap-6">
          <form onSubmit={handleAdd} className="flex gap-2 items-end bg-secondary/20 p-4 rounded-xl border border-border/50 flex-wrap sm:flex-nowrap">
            <div className="flex-1 min-w-[150px] flex flex-col gap-1.5">
              <label className="text-xs font-bold text-muted-foreground uppercase">Região/Bairro</label>
              <Input value={nome} onChange={e => setNome(e.target.value)} placeholder="Ex: Centro" required className="bg-white" />
            </div>
            ...
            <div className="w-24 flex flex-col gap-1.5">
              <label className="text-xs font-bold text-muted-foreground uppercase">Prazo (min)</label>
              <Input type="number" value={prazo} onChange={e => setPrazo(e.target.value)} placeholder="30" className="bg-white" />
            </div>
            <Button type="submit" className="bg-primary text-white font-bold h-10 px-6 mt-2 sm:mt-0 w-full sm:w-auto">Adicionar</Button>
          </form>
              ...
              <tbody className="divide-y divide-border/50">
                {regioesEntrega.map(regiao => (
                  <tr key={regiao.id} className="bg-white hover:bg-secondary/10">
                    <td className="px-4 py-3 font-semibold text-foreground">{regiao.nome}</td>
                ...
                {regioesEntrega.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground">Nenhuma região cadastrada.</td>
                  </tr>
                )}
              </tbody>
            </table>
